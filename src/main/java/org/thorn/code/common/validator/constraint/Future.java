...
/**
 * 支持Date、Calendar，判断时间是否为当前时间之后.
 *
 * @author chenyun.chris
 * @since 2018.05.10
 */
public class Future implements Constraint {

    private static final Logger LOGGER = LoggerFactory.getLogger(Future.class);

    private final String message = "must be in the future";
    ...
    public void check(Object value, String... checkArgs) throws ValidationException {
        ...
        if(value == null || (!(value instanceof Date) && !(value instanceof Calendar))) {
            LOGGER.warn("Future constraint input may not be right!");
            return;
        }
